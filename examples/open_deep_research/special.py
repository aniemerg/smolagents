...
'''
def save_agent_memory(agent, filename):
    """Save all prompts and completions from an agent's memory to a file."""
    import json
    ...
    for step in agent.memory.steps:
        ...
        # Save prompt (model input)
        if hasattr(step, 'model_input_messages') and step.model_input_messages:
            step_data['prompt'] = step.model_input_messages
        
        # Save completion (model output)
        if hasattr(step, 'model_output_message') and step.model_output_message:
            if hasattr(step.model_output_message, 'content'):
                step_data['completion'] = step.model_output_message.content
            
            # Also save raw response if available
            if hasattr(step.model_output_message, 'raw') and step.model_output_message.raw:
                step_data['raw_response'] = str(step.model_output_message.raw)
